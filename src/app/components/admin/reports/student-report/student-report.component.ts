...
searchKey: any = '';
 ...
 onGroupChange(event: any) {
   ...
   onTestChange(event: any) {
    ...
    this.test_id = testId;
    this.getAllStudentListWma(this.test_id);
  }
  getAllStudentReportList() {
    ...
    this.test_id = this.form.value.test_id;
    this.student_id = this.form.value.student_id;
    this._adminService.getAllStudentReport(this.page, this.perPage, this.fromDate, this.toDate, this.group_id, this.test_id, this.student_id, this.searchKey).subscribe({
      next: (res: any) => {
  ...
  downloadAllStudentReportList() {
    ...
    this.group_id = this.form.value.group_id;
    this.test_id = this.form.value.test_id;
    this._adminService.downloadAllStudentReportList(this.fromDate, this.toDate, this.group_id, this.test_id, this.student_id, this.searchKey).subscribe({
      next: (blob: Blob) => {
        const url = window.URL.createObjectURL(blob);
        const link = document.createElement('a');
        link.href = url;
        link.download = 'All-Student-Report.xlsx';  // Set a proper filename
        link.click();
